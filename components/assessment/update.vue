...
watch(
  ...
  (newValue) => {
    if (newValue) {
      ...
      formData.value.title = dataSource.name;
      formData.value.description = dataSource.description;
      formData.value.categories_id = dataSource.categories_id;
      formData.value.rating = dataSource.rating;
      formData.value.difficulty = dataSource.difficulty;
...
const imageURL = computed(() => {
  if (fileSource.value) {
    return URL.createObjectURL(fileSource.value) ?? props.dataSource?.image;
  }
});
...
async function handleSubmit() {
  ...
  useFetchApi(api.updateAssessment, {
    method: "put",
    params: { id },
    body: formData.value,
  })
    .then(() => {
      closeModal();
      emit("submitted");
      triggerAlert("The assessment has been update Successfully", "success");
    })
    .catch((error) => {
      if (error) {
  ...
  >
    ...
    >
      <div class="flex justify-between items-center mb-6">
        <h3 class="text-xl font-bold text-gray-900">Update New Assessment</h3>
